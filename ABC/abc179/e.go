// refs: https://github.com/maitaken/atcoder-template
package main
...
func solve() {
	...
	if cycle_start != -1 {
		cycles := (N - cycle_start) / (cycle_end - cycle_start)
		ends := (N - cycle_start) % (cycle_end - cycle_start)
		//A_before_sum := Sum(A[0:cycle_start])
		A_loop_sum := Sum(A[cycle_start:cycle_end]) * (cycles - 1)
		A_after_sum := Sum(A[cycle_start : cycle_start+ends])
		fmt.Println(temp_sum + A_loop_sum + A_after_sum)
	} else {
		fmt.Println(temp_sum)
	}
}
